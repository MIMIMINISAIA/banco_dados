...
INSERT INTO funcionarios_projetos (id_funcionario,id_projeto,horas_trabalhadas,valor_hora) VALUES
	...
	-- exercicio 1
	select count(fp.id_projeto) as total 
   ...
   -- exercicio 2
   select p.nome as projeto, d.nome as dapartamento, p.valor_total 
   from projetos p 
  ...
  -- exercicio 3
  select nome as funcionario, avg(fp.horas_trabalhadas) as media  
  from funcionarios f 
...
-- exercicio 12
select p.nome as projeto, f.nome as fucionario, sum(fp.horas_trabalhadas) as horas_trabalhadas
from projetos p 
inner join funcionarios_projetos fp on p.id_projeto = fp.id_projeto 
