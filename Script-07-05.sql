...
call inserir_funcionario('Ana', 'ana@gmail.com', '123', 1, 50);


/*
Delimiter //
create function calcula_valor_total_projeto()
returns decimal(5,2)
begin
	   declare calculo decimal(5,2);
	   select sum(horas_trabalhadas) into calculo from projetos p ;
	   return calculo;
end//
delimiter ;


select calcula_valor_total_projeto() as calculoProjetos;
select calcula_valor_total_projeto() as calculoProjetos from projetos p ;

*/

-- exercicio 3
Delimiter //
create function calcular_horas_projeto(id)
returns decimal(5,2)
begin
	   declare calculo decimal(5,2);
	   select sum(horas_trabalhadas) into calculo from funcionarios_projetos fp  
	   where id = calculo;
	   return calculo;
end//
delimiter ;
